...
if [ -d servers/yadr01/panelka/app-configs ]; then
...
if [ "${FORCE_FULL_RESTART:-0}" = "1" ]; then
    ...
    CHANGED_FILES=$(git diff --name-only --diff-filter=AM $OLD_HASH $NEW_HASH -- servers/yadr01/panelka/quadlets/ | grep '\.container$' || true)
