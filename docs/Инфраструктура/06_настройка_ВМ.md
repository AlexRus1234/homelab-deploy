# 06 — Настройка ВМ: Rootless Podman + GitOps-агент
...
```bash
# Проверить статус службы (замените 00-homepage на имя вашего сервиса)
systemctl --user status 00-homepage

# Посмотреть логи контейнера в реальном времени
journalctl --user -fu 00-homepage
